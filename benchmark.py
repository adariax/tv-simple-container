...
ParseFunc = t.Callable[[t.Iterator[str]], list]
...
def parse_avg_stddev(iterator: t.Iterator[str], i: int = 2) -> list:
    return [float(x) for x in next_line(iterator)[i].split("/")]


def parse_general_stats(iterator: t.Iterator[str]) -> list:
    next(iterator)
    return [parse_sec(iterator), parse_float(iterator, i=4)]


def parse_latency_ms(iterator: t.Iterator[str]) -> list:
    next(iterator)
...
def parse_threads_fairness(iterator: t.Iterator[str]) -> list:
    next(iterator)
...
def parse_cpu_eps(iterator: t.Iterator[str]) -> list:
    next(iterator)
    return [parse_float(iterator, i=3)]


def parse_ops_mem_speed(iterator: t.Iterator[str]) -> list:
    ops_per_sec = float(next_line(iterator)[3][1:])
    mem_speed = float(next_line(iterator)[3][1:])
    return [ops_per_sec, mem_speed]


def parse_file_ops(iterator: t.Iterator[str]) -> list:
    next(iterator)
    return [parse_float(iterator) for _ in range(3)]


def parse_throughput(iterator: t.Iterator[str]) -> list:
    next(iterator)
...
def test_primes(iterator: t.Iterator[str]) -> list:
...
def test_threads(iterator: t.Iterator[str]) -> list:
...
def test_memory(iterator: t.Iterator[str]) -> list:
...
def test_fileio(iterator: t.Iterator[str]) -> list:
